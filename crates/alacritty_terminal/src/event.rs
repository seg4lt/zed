...
use crate::vte::ansi::{Progress, Rgb};
...
pub enum Event {
    ...
    /// Terminal progress indicator change.
    Progress(Progress),
...
impl Debug for Event {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Event::Progress(progress) => write!(f, "Progress({progress:?})"),
            Event::Notification(message) => write!(f, "Notification({message})"),
