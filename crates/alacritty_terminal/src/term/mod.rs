...
impl<T: EventListener> Handler for Term<T> {
    #[inline]
    fn set_progress(&mut self, progress: vte::ansi::Progress) {
        self.event_proxy.send_event(Event::Progress(progress));
    }

    #[inline]
    fn terminal_notification(&mut self, message: String) {
        self.event_proxy.send_event(Event::Notification(message));
    }
    ...
    fn reset_state(&mut self) {
        self.event_proxy.send_event(Event::Progress(vte::ansi::Progress::Remove));
        if self.mode.contains(TermMode::ALT_SCREEN) {
            mem::swap(&mut self.grid, &mut self.inactive_grid);
        }
