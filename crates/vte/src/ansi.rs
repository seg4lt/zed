...
pub trait Handler {
    ...
    fn set_progress(&mut self, _: Progress) {}

    /// Show a terminal notification.
    fn terminal_notification(&mut self, _: String) {}
...
/// State communicated by the OSC 9;4 progress sequence.
    ...
    T: Timeout,
{
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            // Set terminal progress indicator.
            b"9" if params.get(1) == Some(&b"4".as_slice()) => {
                let progress = match params.get(2).copied() {
                    None | Some(b"0") => Some(Progress::Remove),
                    Some(b"1") => params
                        .get(3)
                        .and_then(|value| parse_number(value))
                        .filter(|value| *value <= 100)
                        .map(Progress::Set),
                    Some(b"2") => match params.get(3).filter(|value| !value.is_empty()) {
                        Some(value) => parse_number(value)
                            .filter(|value| *value <= 100)
                            .map(|value| Progress::Error(Some(value))),
                ...
                if let Some(progress) = progress {
                    self.handler.set_progress(progress);
                } else {
                    unhandled(params);
                }
            },
            ...
            b"9" if params.len() >= 2 => {
                let message = params[1..]
                    ...
                    .collect::<Vec<_>>()
                    .join(";");
                self.handler.terminal_notification(message);
            },

            // Show desktop notification.
            b"777" if params.get(1) == Some(&b"notify".as_slice()) && params.len() >= 3 => {
...
mod tests {
    ...
    impl Handler for MockHandler {
        ...
        fn terminal_notification(&mut self, message: String) {
    ...
    #[test]
    fn parse_osc9_progress() {
        let cases = [
            (b"\x1b]9;4\x1b\\".as_slice(), Progress::Remove),
            (b"\x1b]9;4;0\x07".as_slice(), Progress::Remove),
            (b"\x1b]9;4;1;42\x1b\\".as_slice(), Progress::Set(42)),
        ...
        for (bytes, expected) in cases {
            ...
            assert_eq!(handler.progress, Some(expected));
        }
    }

    #[test]
    fn reject_invalid_osc9_progress() {
        for bytes in [
            b"\x1b]9;4;1\x1b\\".as_slice(),
            b"\x1b]9;4;1;101\x1b\\".as_slice(),
            b"\x1b]9;4;4;invalid\x1b\\".as_slice(),
            b"\x1b]9;4;5;20\x1b\\".as_slice(),
        ] {
            let mut parser = Processor::<TestSyncHandler>::new();
            let mut handler = MockHandler::default();
            parser.advance(&mut handler, bytes);
            assert_eq!(handler.progress, None);
        }
    }
    ...
    fn parse_osc9_notification() {
        ...
        parser.advance(&mut handler, b"\x1b]9;Approval requested: cargo test\x07");

        assert_eq!(handler.notification.as_deref(), Some("Approval requested: cargo test"));
        assert_eq!(handler.progress, None);
    }
    ...
    fn parse_osc777_notification() {
        ...
        parser.advance(
            &mut handler,
            b"\x1b]777;notify;Permission needed;Claude needs your attention\x1b\\",
        );

        assert_eq!(
            handler.notification.as_deref(),
            Some("Permission needed: Claude needs your attention")
        );
    }
