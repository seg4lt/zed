...
impl<T: EventListener> Handler for Term<T> {
    ...
    #[inline]
    fn terminal_notification(&mut self, title: Option<String>, body: Option<String>) {
        trace!("Terminal notification title={title:?} body={body:?}");
        self.event_proxy
            .send_event(Event::TerminalNotification { title, body });
    }
...
mod tests {
    ...
    #[derive(Clone)]
    struct TestListener(Arc<Mutex<Vec<Event>>>);
    ...
    fn parse_events(input: &[u8]) -> Vec<Event> {
        ...
        let listener = TestListener(events.clone());
    ...
    #[test]
    fn osc_9_emits_terminal_notification_event() {
        let events = parse_events(b"\x1b]9;Task done\x07");
    ...
    #[test]
    fn osc_99_emits_terminal_notification_event() {
        let events = parse_events(b"\x1b]99;i=1:d=0;Claude Code;Waiting for input\x07");
        assert!(
            events
                .iter()
                .any(|event| matches!(event, Event::TerminalNotification { .. }))
        );
    }

    #[test]
    fn osc_777_emits_terminal_notification_event() {
        let events = parse_events(b"\x1b]777;notify;Claude Code;Waiting for input\x07");
        assert!(events.iter().any(|event| matches!(
            event,
            Event::TerminalNotification {
                title: Some(title),
                body: Some(body)
            } if title == "Claude Code" && body == "Waiting for input"
        )));
    }

    #[test]
    fn terminal_notification_osc_does_not_render_cells() {
        let size = TermSize::new(10, 10);
        let mut term = Term::new(Config::default(), &size, VoidListener);
        let mut parser = crate::vte::ansi::Processor::<crate::vte::ansi::StdSyncHandler>::new();
        parser.advance(
            &mut term,
            b"\x1b]777;notify;Claude Code;Waiting for input\x07",
        );
    ...
    #[test]
    fn title_and_clipboard_osc_still_work() {
        let events = parse_events(b"\x1b]0;My Title\x07\x1b]52;c;QQ==\x07");

        assert!(
            events
                .iter()
                .any(|event| matches!(event, Event::Title(title) if title == "My Title"))
        );
        assert!(events.iter().any(|event| matches!(
            event,
            Event::ClipboardStore(_, text) if text == "A"
        )));
    }
}
