...
pub trait Handler {
    ...
    /// Terminal desktop notification (OSC 9 / 99 / 777).
    fn terminal_notification(&mut self, _: Option<String>, _: Option<String>) {}
    ...
    T: Timeout,
{
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        fn join_osc_text(params: &[&[u8]]) -> Option<String> {
            if params.is_empty() {
                return None;
            }
        ...
        match params[0] {
            ...
            b"9" => {
                let body = join_osc_text(&params[1..]);
                self.handler.terminal_notification(None, body);
            },
            ...
            // Desktop notification (kitty).
            b"99" => {
                // Kitty can prefix with metadata/options before title/body. We only need a
                // best-effort extraction since the terminal consumer currently uses this as an
                // attention signal.
                let has_options = params.get(1).is_some_and(|first| first.contains(&b'='));

                let title = if has_options {
                    params.get(2).and_then(|param| join_osc_text(&[*param]))
                } else {
                ...
                self.handler.terminal_notification(title, body);
            },
            ...
            // Desktop notification (Ghostty / urxvt extension).
            b"777" => {
                if params.get(1).is_some_and(|param| *param == b"notify") {
                    let title = params.get(2).and_then(|param| join_osc_text(&[*param]));
                    let body = if params.len() > 3 { join_osc_text(&params[3..]) } else { None };
                    self.handler.terminal_notification(title, body);
                } else {
                    unhandled(params);
                }
